...
public class ModifiedWiringPiGpioExample {
    ...
    public static void main(String args[]) throws InterruptedException {
        ...
        for (pin = 0; pin < 7; ++pin) {
        	...
        	wiringpiConnector.export(pin, GpioUtil.DIRECTION_OUT);            
        	wiringpiConnector.pinMode(pin, Gpio.OUTPUT);
        }
        ...
        for (;;) {
            ...
            wiringpiConnector.digitalWrite(l, s);
            
            if (wiringpiConnector.digitalRead(7) == 1) // Pressed as our switch shorts to ground
            	wiringpiConnector.delay(d * 10); // Faster!
            else
            	wiringpiConnector.delay(d * 100);
        }
    }
}
